<x-app-layout :title="'Editar evento'">
<div class="max-w-xl mx-auto py-8">
    ...
    <form method="POST" action="{{ route('events.update', $event) }}" class="space-y-4" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div>
            ...
            @if($event->cover_image)
                <img src="/{{ $event->cover_image }}" alt="Portada actual" class="mt-2 rounded w-32 h-20 object-cover">
            @endif
            @error('cover_image')<div class="text-error text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block font-semibold mb-1">Título</label>
            <input type="text" name="title" class="input input-bordered w-full" value="{{ old('title', $event->title) }}" required>
            @error('title')<div class="text-error text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block font-semibold mb-1">Descripción</label>
            <textarea name="description" class="textarea textarea-bordered w-full" rows="4" required>{{ old('description', $event->description) }}</textarea>
            @error('description')<div class="text-error text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block font-semibold mb-1">Fecha</label>
            <input type="date" name="date" class="input input-bordered w-full" value="{{ old('date', $event->date ? $event->date->format('Y-m-d') : '' ) }}" required min="{{ date('Y-m-d') }}">
